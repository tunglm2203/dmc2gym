...
from dmc2gym.distracting_control import background
from dmc2gym.distracting_control import camera
from dmc2gym.distracting_control import color
from dmc2gym.distracting_control import suite_utils
...
def load(domain_name,
    ...
    Args:
      ...
      default_background: If set, use default background from DMControl
      default_camera: If set, use default camera view from DMControl
      default_color: If set, use default robot's color from DMControl

    Returns:
      The requested environment.
    """

    assert background_dataset_videos in ['train', 'training', 'val', 'validation'], \
        "`background_dataset_videos` must be one of ['train', 'training', 'val', 'validation']"
    ...
    if (difficulty or background_kwargs) and not default_background:
    ...
    if (difficulty or camera_kwargs) and not default_camera:
    ...
    if (difficulty or color_kwargs) and not default_color:
    ...
    # TUNG: comment this, since it returns the observation including undesired 'pixel' key
    # env = pixels.Wrapper(
    #     env,
